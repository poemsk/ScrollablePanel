package ninja.poepoe.scrollablepanel;
...
public class ScrollablePanelAdapter extends PanelAdapter {
  ...
  @Override public void onBindViewHolder(RecyclerView.ViewHolder holder, int row, int column) {
    ...
    if (orderInfo != null) {
      if (orderInfo.getStatus() == OrderInfo.Status.BLANK) {
        ...
        viewHolder.view.setBackgroundResource(
            orderInfo.isBegin() ? R.drawable.bg_room_red_begin_with_stroke
                : R.drawable.bg_room_red_with_stroke);
      } else if (orderInfo.getStatus() == OrderInfo.Status.REVERSE) {
        viewHolder.nameTextView.setText(orderInfo.isBegin() ? orderInfo.getGuestName() : "");
        viewHolder.statusTextView.setText(orderInfo.isBegin() ? "reverse" : "");
        viewHolder.view.setBackgroundResource(
            orderInfo.isBegin() ? R.drawable.bg_room_blue_begin_with_stroke
                : R.drawable.bg_room_blue_middle);
      }
      if (orderInfo.getStatus() != OrderInfo.Status.BLANK) {
        ...
        viewHolder.itemView.setOnClickListener(new View.OnClickListener() {
          @Override public void onClick(View v) {
            if (orderInfo.isBegin()) {
              Toast.makeText(v.getContext(), "name:" + orderInfo.getGuestName(), Toast.LENGTH_SHORT)
                  .show();
            } else {
              int i = 2;
              while (column - i >= 0 && ordersList.get(row - 1).get(column - i).getId() == orderInfo
                  .getId()) {
                i++;
              }
              final OrderInfo info = ordersList.get(row - 1).get(column - i + 1);
              Toast.makeText(v.getContext(), "name:" + info.getGuestName(), Toast.LENGTH_SHORT)
                  .show();
            }
          }
        });
